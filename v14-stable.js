#!/usr/bin/env node
/**
 * Site X-Ray v14 — Universal precision cloner.
 * Builds on v13 with:
 *   - Clean context recovery for pages where init script destroys DOM
 *   - www/non-www domain variant URL mapping (fixes cross-domain image refs)
 *   - Favicon/icon download from <link> tags and common paths
 *   - Final external URL cleanup pass for both domain variants
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v14-stable.js <url> [output-dir] [max-pages] [flags]
...
Examples:
  node v14-stable.js https://example.com
  node v14-stable.js https://example.com ./output 50
  node v14-stable.js https://example.com --all
  node v14-stable.js https://example.com --auth auth-state.json
  node v14-stable.js https://example.com --save-auth
  node v14-stable.js https://example.com --interactive`);
  process.exit(0);
}
...
// v14: www/non-www domain variant — sites may reference assets from either variant
const ALT_DOMAIN = (() => {
...
function mapAsset(orig, local) {
  ...
  try {
    ...
    // v14: Also map the www/non-www alternate domain variant
...
async function capturePage(page, urlPath, isFirst) {
  ...
  if (isFirst) { try {
    ...
    // v14: Download common favicon/icon files and discover from <link> tags
    try {
      const commonIcons = ['/apple-touch-icon.png','/favicon-32x32.png','/favicon-16x16.png','/favicon.svg','/apple-touch-icon-precomposed.png'];
      for (const iconPath of commonIcons) {
        const dest = `${OUT}${iconPath}`;
        if (!fs.existsSync(dest)) await dl(`${DOMAIN}${iconPath}`, dest);
      }
      // Also discover icons from <link> tags in the page
      const iconLinks = await page.evaluate(() => {
        return [...document.querySelectorAll('link[rel*="icon"], link[rel*="apple-touch"]')]
          .map(l => l.getAttribute('href'))
          .filter(Boolean);
      });
      for (const href of iconLinks) {
        try {
          const absUrl = new URL(href, DOMAIN).href;
          const iconPath = new URL(absUrl).pathname;
          const dest = `${OUT}${iconPath}`;
          if (!fs.existsSync(dest)) {
            fs.mkdirSync(path.dirname(dest), { recursive: true });
            await dl(absUrl, dest);
          }
        } catch {}
      }
    } catch {}
  ...
  // v14: Also handle www/non-www variant in CSS
  ...
  html = html.split(DOMAIN + '"').join('/"');
  // v14: Also handle www/non-www variant
...
async function main() {
  ...
  if (flags.interactive || flags.saveAuth) {
    ...
    if (flags.saveAuth) {
      ...
      console.log(`   Reuse with: node v14-stable.js ${TARGET} --auth ${stateFile}\n`);
    }
  ...
  // ═══════════════════════════════════════
  // v14: Clean context recovery — if init script destroyed the DOM, re-capture without it
  ...
  try {
    ...
    if (indexContent.length < 500 || bodyContent.length < 100) {
      ...
      try {
        ...
        // Get the HTML FIRST before any DOM modifications
...
document.querySelectorAll('button,a,[role="button"]').forEach(el=>{el.style.pointerEvents='auto';el.style.cursor='pointer'});
  ...
  for (const file of htmlFiles) {
    ...
    while ((m = relRegex.exec(h)) !== null) {
      const p = m[1];
      // v14: Also include .html links if the file doesn't exist (clean recovery pages)
...
${stubFooter || ''}
  ...
  for (const file of allHtmlFiles) {
    ...
    h = h.replace(/href="(\/[^"]*?)"/g, (match, linkPath) => {
      // v14: Check .html links — if file doesn't exist, generate stub instead of skipping
      if (linkPath.endsWith('.html') && fs.existsSync(path.join(OUT, linkPath))) return match;
      if (linkPath.endsWith('.css') || linkPath.endsWith('.js')) return match;
  ...
  // v14: Final external URL cleanup — strip remaining references to both domain variants
  // Only operates on href/src attributes and url() in CSS, not free text
  let externalCleaned = 0;
  for (const file of allHtmlFiles) {
    let h = fs.readFileSync(file, 'utf-8');
    const before = h;
    // Strip both DOMAIN and ALT_DOMAIN from href/src attributes
    for (const dom of [DOMAIN, ALT_DOMAIN]) {
      const escaped = dom.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      // Convert absolute URLs to relative paths in attributes
      h = h.replace(new RegExp(`((?:src|href|action|poster)=["'])${escaped}/`, 'g'), '$1/');
      h = h.replace(new RegExp(`((?:src|href|action|poster)=["'])${escaped}"`, 'g'), '$1/"');
      // Also handle protocol-relative URLs
      const protoRel = dom.replace(/^https?:/, '');
      const protoRelEsc = protoRel.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      h = h.replace(new RegExp(`((?:src|href|action|poster)=["'])${protoRelEsc}/`, 'g'), '$1/');
      // Clean url() references in inline styles
  ...
  if (externalCleaned) console.log(`     Cleaned external refs in ${externalCleaned} files`);
