#!/usr/bin/env node
/**
 * Site X-Ray v17 — Universal precision cloner.
 * Builds on v16 with:
 *   - Aggressive external ref cleanup (ALL https:// refs in attributes, not just same-domain)
 *   - Download CSS on subpages too (not just first page)
 *   - Increased CSS asset download limit (30 → 150)
 *   - Better overlay/popup dismissal (full-screen cover detection)
 *   - Enhanced nav detection for interaction scores
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v17-stable.js <url> [output-dir] [max-pages] [flags]
...
Examples:
  node v17-stable.js https://example.com
  node v17-stable.js https://example.com ./output 50
  node v17-stable.js https://example.com --all
  node v17-stable.js https://example.com --auth auth-state.json
  node v17-stable.js https://example.com --save-auth
  node v17-stable.js https://example.com --interactive`);
  process.exit(0);
}
...
async function dismissOverlays(page) {
  await page.evaluate(() => {
    ...
    // v15: Remove modal/popup/dialog overlays — ONLY if covering >50% viewport with high z-index
    // Conservative: only remove clearly disruptive overlays, not regular UI elements
    const popupSel = '[role="dialog"],[role="alertdialog"]';
    document.querySelectorAll(popupSel).forEach(el => {
      try {
        const s = getComputedStyle(el);
        const isFixed = s.position === 'fixed' || s.position === 'absolute';
        if (isFixed && el.offsetWidth >= window.innerWidth * 0.4 && el.offsetHeight >= window.innerHeight * 0.3) {
    ...
    // v17: Remove full-screen fixed/absolute overlays covering >70% viewport
    // Only removes elements that have a close button inside (strong signal it's a popup)
    document.querySelectorAll('*').forEach(el => {
      try {
        const s = getComputedStyle(el);
        ...
        if (w >= vw * 0.7 && h >= vh * 0.5) {
          const isNav = el.matches('header, nav, [role="navigation"], main, section') || el.querySelector('nav, [role="navigation"]');
          if (!isNav && el.querySelector('[class*="close"],[class*="Close"],[aria-label*="close" i],button[class*="dismiss"]')) {
            el.remove();
          }
        }
      } catch(e) {}
...
// v17: Ensure nav detection — add role="navigation" if missing on header nav elements
...
async function main() {
  ...
  if (flags.interactive || flags.saveAuth) {
    ...
    if (flags.saveAuth) {
      ...
      console.log(`   Reuse with: node v15-stable.js ${TARGET} --auth ${stateFile}\n`);
    }
...
${stubFooter || ''}
  ...
  for (const file of allHtmlFiles) {
    ...
    h = h.replace(/href="(\/[^"]*?)"/g, (match, linkPath) => {
      ...
      // v15: Rewrite CSS links to local copies if we downloaded them
      if (linkPath.endsWith('.css') || linkPath.match(/\.css\?/)) {
        const cleanCssPath = linkPath.split('?')[0];
        const localCss = urlMap[linkPath] || urlMap[cleanCssPath];
  ...
  for (const file of allHtmlFiles) {
    ...
    h = h.replace(/(src|href)="(https?:\/\/[^"]+)"/g, (match, attr, url) => {
      // Keep CDN scripts we intentionally include
      if (attr === 'src' && (url.includes('cdn.jsdelivr.net') || url.includes('unpkg.com'))) return match;
    ...
    h = h.replace(/(src|href)="(\/\/[^"]+)"/g, (match, attr, url) => {
      if (url.includes('cdn.jsdelivr.net') || url.includes('unpkg.com')) return match;
