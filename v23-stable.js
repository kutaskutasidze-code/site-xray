#!/usr/bin/env node
/**
 * Site X-Ray v23 — Universal precision cloner.
 * Builds on v22 with METRIC-FOCUS on pixels:
 *   - Resolve <picture><source> elements → best src on <img>, remove <source> (fixes broken images)
 *   - Fix lazy images with placeholder src + data-src (duplicate src bug)
 *   - Reveal content elements with CSS opacity:0 that JS would normally animate
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v23-stable.js <url> [output-dir] [max-pages] [flags]
...
Examples:
  node v23-stable.js https://example.com
  node v23-stable.js https://example.com ./output 50
  node v23-stable.js https://example.com --all
  node v23-stable.js https://example.com --auth auth-state.json
  node v23-stable.js https://example.com --save-auth
  node v23-stable.js https://example.com --interactive`);
  process.exit(0);
}
...
// v23: Reveal content elements hidden by CSS opacity:0 (JS entrance animations stripped)
...
// v23: Reveal content elements hidden by CSS opacity:0
try{
