#!/usr/bin/env node
/**
 * Site X-Ray v42 — Universal precision cloner.
 * Builds on v41 with METRIC-FOCUS improvement:
 *   - Freeze CSS animations early to prevent carousel auto-advance (pixel fidelity)
 *   - Early viewport screenshot for ALL sites as hero background fallback
 *   - Previous: src-less img fix, early viewport screenshot, multi-canvas fallback
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v42-stable.js <url> [output-dir] [max-pages] [flags]
...
if (!TARGET) {
  console.log(`Site X-Ray v41
Usage: node v41-stable.js <url> [output-dir] [max-pages] [flags]
...
*,*::before,*::after{animation-duration:0.001s!important;animation-delay:0s!important;animation-iteration-count:1!important;animation-fill-mode:both!important}</style>
...
<style>${css}</style>
<style>html,body{overflow-y:auto!important;overflow-x:hidden!important;scroll-behavior:smooth}html{scrollbar-width:none}html::-webkit-scrollbar{display:none}body{background-color:${capturedBodyBg || '#ffffff'};font-feature-settings:normal;text-rendering:optimizeLegibility}img[src=""]{display:none}button,a,[role="button"]{pointer-events:auto!important;cursor:pointer!important}
/* v42: Freeze CSS animations in output HTML */
*,*::before,*::after{animation-play-state:paused!important;animation-delay:0s!important;animation-iteration-count:1!important}</style>
<link rel="icon" href="/favicon.ico"/>
</head>`);
