#!/usr/bin/env node
/**
 * Site X-Ray v47 — Universal precision cloner.
 * Builds on v46:
 *   - PIXEL FIX: Set scroll-driven --progress to 1 (not 0) to reveal content
 *   - Previous: SVG fragment fix, CSS/GSAP animation freeze
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v47-stable.js <url> [output-dir] [max-pages] [flags]
...
Examples:
  node v47-stable.js https://example.com
  node v47-stable.js https://example.com ./output 50
  node v47-stable.js https://example.com --all
  node v47-stable.js https://example.com --auth auth-state.json
  node v47-stable.js https://example.com --save-auth
  node v47-stable.js https://example.com --interactive`);
  process.exit(0);
}
...
async function capturePage(page, urlPath, isFirst) {
  ...
  // v47: PIXEL FIX — Bake computed background-image into inline styles before DOM capture.
  // Creative sites set background-image via JS (e.g. Next.js Image, dynamic CSS classes).
  // page.content() captures inline styles, but computed background-images from JS-applied
  // CSS classes may reference URLs that get lost after script removal.
  // Only bakes on elements that have a real url() background but no inline background-image.
  try {
    await page.evaluate(() => {
      document.querySelectorAll('img, [class*="cover"], [class*="thumb"], [class*="card"], [class*="project"], [class*="slide"], [class*="hero"], [class*="media"], [class*="image"], [class*="poster"], [class*="visual"]').forEach(el => {
        try {
          if (el.style.backgroundImage) return; // already has inline
          const bg = getComputedStyle(el).backgroundImage;
...
${stubFooter || ''}
  ...
  for (const file of allHtmlFiles) {
    ...
    h = h.replace(/href="(\/[^"]*?)"/g, (match, linkPath) => {
      ...
      // v46: Handle SVG fragment references (e.g. /images/sprite.svg#icon)
      // These break the link checker because the #fragment is treated as part of the file path.
      // Strip the file path, keep only the #fragment (SVG defs were already inlined by capturePage).
      if (linkPath.match(/\.svg#/i)) {
        const fragment = '#' + linkPath.split('#').pop();
        return `href="${fragment}"`;
      }
      // v46: For asset paths with #fragment, strip fragment before file check
      if (linkPath.includes('#') && linkPath.match(/\.(jpg|png|svg|webp|pdf|ico|woff|woff2|mp4)(\?|#)/i)) {
        const assetPath = linkPath.split('?')[0].split('#')[0];
        if (fs.existsSync(path.join(OUT, assetPath))) {
          return `href="${assetPath}"`;
        }
        return match;
      }
  ...
  // v46: Final SVG fragment cleanup — strip file paths from any remaining SVG use references
  // The link checker treats href="/file.svg#id" as a file path (not an anchor), causing broken links.
  // This catches xlink:href and any other refs missed by the href= rewriter above.
  try {
    let svgFixCount = 0;
    for (const file of allHtmlFiles) {
      let h = fs.readFileSync(file, 'utf-8');
      const before = h;
      // Match both href= and xlink:href= pointing to .svg#fragment
      h = h.replace(/((?:xlink:)?href=")([^"]*\.svg)(#[^"]+)"/g, (match, prefix, svgPath, fragment) => {
    ...
    if (svgFixCount > 0) console.log(`     v46: Fixed SVG fragment refs in ${svgFixCount} files`);
  } catch(e) {}
