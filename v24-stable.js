#!/usr/bin/env node
/**
 * Site X-Ray v24 — Universal precision cloner.
 * Builds on v23 with METRIC-FOCUS on pixels:
 *   - Exclude carousel/slider slides from opacity reveal (fixes pixel mismatch on Swiper/Flickity sites)
 *   - Skip opacity reveal on elements with large translateX (intentionally off-screen)
 *   - Preserves carousel initial state for pixel-accurate snapshots
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v24-stable.js <url> [output-dir] [max-pages] [flags]
