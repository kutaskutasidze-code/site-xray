...
async function capturePage(page, urlPath, isFirst) {
  ...
  if (isFirst) {
    ...
    // Canvas capture — identify WebGL canvases for pipeline-level frame capture
    ...
    const webglCanvases=canvasInfo.filter(c=>c.ctxType!=='2d'&&c.visible);

    // Static capture for all canvases
    const canvases=await page.$$('canvas');
    for(let i=0;i<canvases.length;i++){try{const du=await canvases[i].evaluate(c=>{try{return c.toDataURL('image/png')}catch{return null}});if(du&&du.length>500)fs.writeFileSync(`${OUT}/images/canvas-${i}.png`,Buffer.from(du.split(',')[1],'base64'));else await canvases[i].screenshot({path:`${OUT}/images/canvas-${i}.png`});}catch(e){}}

    // Pipeline-level WebGL capture: read framebuffer directly via toDataURL (preserveDrawingBuffer enabled)
    if(webglCanvases.length>0){
      console.log(`     WebGL canvases: ${webglCanvases.length} (pipeline capture)...`);
      for(const wc of webglCanvases){
        ...
        for(let f=0;f<frameCount;f++){
          ...
          await page.waitForTimeout(50); // ~20fps
        }
    ...
    if (bundleLib.includes('gsap') || bundleLib.includes('scrolltrigger') || bundleLib.includes('lenis') || bundle.frameSequences.length > 0) {
      ...
      if (bundle.frameSequences.length > 0) {
        ...
        // Match each sequence to a canvas on the page
        // Bind frame sequences to their canvases (lazy-load to avoid lag)
        animScript += `// Bind sequences to canvases (lazy-loaded)\n`;
        animScript += `const seqCanvases = document.querySelectorAll("canvas");\n`;
        bundle.frameSequences.forEach((seq, i) => {
          animScript += `(()=>{\n`;
          animScript += `  const c = seqCanvases[${i + 1}]; if(!c)return;\n`;
          ...
          if (bundleLib.includes('gsap') || bundleLib.includes('scrolltrigger')) {
            animScript += `    ScrollTrigger.create({trigger:c.closest("section")||c.parentElement,start:"top bottom",end:"bottom top",scrub:1,\n`;
            animScript += `      onUpdate:s=>player.drawFrame(Math.round(s.progress*(${seq.count}-1)))});\n`;
          } else {
            animScript += `    const sec=c.closest("section")||c.parentElement;\n`;
            animScript += `    window.addEventListener("scroll",()=>{const r=sec.getBoundingClientRect();const p=Math.max(0,Math.min(1,(innerHeight-r.top)/(innerHeight+r.height)));player.drawFrame(Math.round(p*(${seq.count}-1)))},{passive:true});\n`;
          }
          animScript += `  },{threshold:0.1});\n`;
          animScript += `  obs.observe(c.closest("section")||c.parentElement);\n`;
          animScript += `})();\n`;
        });
      }

      // Bind WebGL-captured canvases (auto-play loop, no scroll dependency)
      if (webglCanvases && webglCanvases.length > 0) {
        animScript += `// WebGL pipeline captures (auto-play loop)\n`;
        webglCanvases.forEach(wc => {
          const seqDir = `${OUT}/sequences/webgl-${wc.i}`;
          if (!fs.existsSync(seqDir)) return;
          const frameFiles = fs.readdirSync(seqDir).filter(f => f.endsWith('.webp'));
          if (frameFiles.length < 5) return;
          animScript += `(()=>{\n`;
          ...
          animScript += `  const obs=new IntersectionObserver(([e])=>{if(e.isIntersecting){obs.disconnect();loop()}},{threshold:0.1});\n`;
          animScript += `  obs.observe(c.closest("section")||c.parentElement);\n`;
          animScript += `})();\n`;
        });
      }
  ...
  // Canvas handling: keep canvases for frame sequences, replace others with video/image
  let ci=0;
  const hasFrameSeqs = fs.existsSync(`${OUT}/sequences`);
  ...
  html=html.replace(/<canvas[^>]*>[\s\S]*?<\/canvas>/g,(match)=>{
    const idx=ci++;
    // If we have frame sequences, keep canvases alive (they'll be driven by JS)
    if(hasFrameSeqs)return match;
    // Otherwise replace with video or static image
