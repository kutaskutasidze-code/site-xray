#!/usr/bin/env node
/**
 * Site X-Ray v15 — Universal precision cloner.
 * Builds on v15 with:
 *   - Download external CSS files as local assets (fixes broken link refs to CSS)
 *   - Aggressive external reference cleanup (data attrs, meta, srcset, JSON-LD)
 *   - Improved overlay/popup dismissal (lower z-index threshold, modal patterns)
 *   - Download favicons from exact HTML-referenced paths (deep dirs, query strings)
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v15-stable.js <url> [output-dir] [max-pages] [flags]
...
Examples:
  node v15-stable.js https://example.com
  node v15-stable.js https://example.com ./output 50
  node v15-stable.js https://example.com --all
  node v15-stable.js https://example.com --auth auth-state.json
  node v15-stable.js https://example.com --save-auth
  node v15-stable.js https://example.com --interactive`);
  process.exit(0);
}
  ...
  for (const cssLink of cssLinks) {
    try {
      ...
      if (res && res.ok) {
        ...
        // v15: Save CSS file locally so <link> tags resolve
        try {
          const cssFileName = `style-${cssFileC}.css`;
          const localCssPath = `/css/${cssFileName}`;
          fs.mkdirSync(`${OUT}/css`, { recursive: true });
          fs.writeFileSync(`${OUT}/css/${cssFileName}`, css);
          // Map the original href (both absolute and relative forms)
          mapAsset(cssLink.href, localCssPath);
          if (cssLink.origHref) {
            mapAsset(cssLink.origHref, localCssPath);
            // Also map the pathname directly (for link rewriting)
            try {
...
${stubFooter || ''}
  ...
  // Expanded to handle data attributes, meta content, srcset, JSON-LD, etc.
  ...
  for (const file of allHtmlFiles) {
    ...
    for (const dom of [DOMAIN, ALT_DOMAIN]) {
      ...
      // v15: Broad attribute cleanup — any attribute containing the domain URL
      ...
      // v15: Clean domain in hash-only URLs (e.g., href="https://domain.com#section")
      h = h.replace(new RegExp(`(\\w+)="([^"]*?)${escaped}(#[^"]*?)"`, 'g'), '$1="$2/$3"');
    }
    if (h !== before) {
      fs.writeFileSync(file, h);
