# Analysis: https://cuberto.com
**Category:** webflow | **Score:** 88/100 | **Perfect:** false

## Tech Stack (Original)
- Framework: Unknown
- CSS: Embedded <style>
- Images: srcset/responsive
- JS libs: none detected
- Scripts: 3 | Stylesheets: 0
- WebGL: no | SVGs: 9 | Iframes: 0

## Element Comparison (Original → Clone)
| Element | Original | Clone | Delta |
|---------|----------|-------|-------|
| Visible | 473 | 473 | 0 |
| Images (rendered) | 17 | 17 | 0 |
| Links | 49 | 49 | 0 |
| Buttons | 1 | 1 | 0 |
| Headings | 7 | 7 | 0 |
| Grids | 15 | 15 | 0 |
| Flexes | 13 | 13 | 0 |

## Scores
| Metric | Score | Perfect |
|--------|-------|---------|
| Images | 100/100 | YES |
| CSS | 100/100 | YES |
| Links | 100/100 | NO |
| Content | 100/100 | YES |
| Layout | 100/100 | YES |
| Pixels | 68/100 | NO |
| Interactions | 40/100 | NO |
| Console | 100/100 | YES |
| Manifest | 70/100 | NO |

## Pixel Comparison
- Match: 68.3%
- Mismatched pixels: 410913 / 1296000
- Diff image: `cuberto-com-diff.png`

## Capture Manifest
- HTML pages: 34
- Images: 69 (avg 227982 bytes)
- Fonts: 16
- Total size: 341486 KB
- External refs remaining: 374
- Issues: 
  - Tiny image (likely broken): img-23.gif (43b)
  - 374 external references still in HTML (should be 0)

## Root Causes
- INTERACTIONS: 0/1 buttons clickable, nav: true

## Suggested Fixes (universal)
- No specific suggestions — site scores well

## Screenshots
- Original: `cuberto-com-original.png`
- Clone: `cuberto-com-clone.png`
- Diff: `cuberto-com-diff.png`
