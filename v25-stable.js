 ...
 * Builds on v24 with METRIC-FOCUS on pixels:
 *   - Freeze all CSS animations/transitions in clone (prevents animation frame drift)
 *   - Capture CSS custom properties from :root/html/body (preserves JS-set themes)
 *   - Freeze clip-path on visible content elements (reveals animation end state)
 *
...
async function capturePage(page, urlPath, isFirst) {
  ...
  // v25: Capture CSS custom properties from :root/html/body before DOM capture
  // Many sites set theme variables via JS (dark mode, colors, spacing).
  // When JS is stripped in the clone, these variables revert to defaults.
  // Inline them so the clone preserves the JS-set theme state.
  await page.evaluate(() => {
    try {
      [document.documentElement, document.body].forEach(el => {
        ...
        const vars = [];
        // Get all CSS custom properties from computed style
  ...
  // v25: Freeze clip-path on visible content elements — reveals animation end state
  // Sites use clip-path animations (e.g., inset(100%) → inset(0%)) for entrance effects.
  // In the original, JS completes the animation. In the clone, elements stay clipped.
  await page.evaluate(() => {
    try {
      document.querySelectorAll('section,article,div,h1,h2,h3,h4,p,figure,header,main').forEach(el => {
        const cs = getComputedStyle(el);
        const cp = cs.clipPath || cs.webkitClipPath;
        if (cp && cp !== 'none') {
          // Skip elements that are intentionally clipped (small decorative elements)
          const r = el.getBoundingClientRect();
          if (r.width < 20 || r.height < 10) return;
          // Only unclip if the clip-path significantly hides content
          // inset() with non-zero values, polygon that clips significantly, circle that's too small
          if (/inset\([^0)][^)]*\)/.test(cp) || /circle\(\s*0/.test(cp)) {
  ...
  // v25: Freeze animated visual state before DOM capture — inline the CURRENT computed
  // opacity/transform/background of elements with running CSS animations.
  // This captures the exact animation frame so the clone shows the same visual state.
  await page.evaluate(() => {
    try {
      document.querySelectorAll('*').forEach(el => {
        ...
        const cs = getComputedStyle(el);
        // Inline key visual properties that animations may be modifying
        const props = ['opacity', 'transform', 'background-color', 'color', 'filter'];
        const overrides = [];
        for (const prop of props) {
          const val = cs.getPropertyValue(prop);
          if (val && val !== '' && val !== 'none' && val !== 'rgba(0, 0, 0, 0)') {
...
<style>${css}</style>
<style>html,body{overflow-y:auto!important;overflow-x:hidden!important;scroll-behavior:smooth}html{scrollbar-width:none}html::-webkit-scrollbar{display:none}body{background-color:${capturedBodyBg || '#ffffff'};font-feature-settings:normal;text-rendering:optimizeLegibility}img[src=""]{display:none}
/* v25: Freeze CSS transitions — prevents on-load transition glitches in clone */
...
async function main() {
  ...
  if (flags.interactive || flags.saveAuth) {
    ...
    if (flags.saveAuth) {
      ...
      console.log(`   Reuse with: node v25-stable.js ${TARGET} --auth ${stateFile}\n`);
    }
