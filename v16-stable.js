#!/usr/bin/env node
/**
 * Site X-Ray v16 — Universal precision cloner.
 * Builds on v15 with:
 *   - Download CSS-referenced assets (fonts, images from url() in stylesheets)
 *   - Rewrite absolute URLs in downloaded CSS to local paths
 *   - Comprehensive external ref cleanup (srcset, inline styles, all attributes)
 *   - Better nav detection and button interactivity
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v16-stable.js <url> [output-dir] [max-pages] [flags]
...
async function downloadExternalCSS(page) {
  ...
  // v16: Download font/image assets referenced in CSS — only same-origin, limit 30, 8s timeout
  const cssAssetMap = {};
  const uniqueUrls = [...new Set(cssAssetUrls)].filter(u => {
    try { return new URL(u).origin === DOMAIN || new URL(u).origin === ALT_DOMAIN; } catch { return false; }
  }).slice(0, 30);
