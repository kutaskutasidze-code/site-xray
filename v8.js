...
// Module-level state shared between capturePage and main
let canvasClassification=[];
let webglContent=[];
...
async function capturePage(page, urlPath, isFirst) {
  ...
  if (isFirst) {
    ...
    // ══════════════════════════════════════════════════════════════
    // UNIVERSAL CANVAS SYSTEM — classify by behavior, not index
    // ══════════════════════════════════════════════════════════════
    canvasClassification=await page.evaluate(()=>{
      return [...document.querySelectorAll('canvas')].map((c,i)=>{
        const r=c.getBoundingClientRect();
        const cs=getComputedStyle(c);
        const pcs=c.parentElement?getComputedStyle(c.parentElement):{};
        let ctxType='2d';
        try{if(c.getContext('webgl2'))ctxType='webgl2';else if(c.getContext('webgl'))ctxType='webgl'}catch{}
        const isVisible=r.width>50&&r.height>50;
        const isFullscreen=r.width>=window.innerWidth*0.95&&r.height>=window.innerHeight*0.95;
        const isFixed=cs.position==='fixed'||pcs.position==='fixed';
        const isHighZ=parseInt(cs.zIndex||pcs.zIndex||'0')>100;
        const isPointerNone=cs.pointerEvents==='none'||pcs.pointerEvents==='none';
        // Role classification
        let role='content';
        if(isFixed&&isFullscreen&&isPointerNone)role='overlay';
        else if(isFixed&&isFullscreen)role='background';
        else if(!isVisible)role='hidden';
        else if(ctxType!=='2d')role='webgl-content';
        // Selector for this canvas (stable reference)
        const parentCls=(c.parentElement?.className||'').split(/\s+/).slice(0,3).join('.');
        const selector=parentCls?`.${parentCls} > canvas`:`:nth-child(${i+1})`;
        return{i,w:Math.round(r.width),h:Math.round(r.height),top:Math.round(r.top),ctxType,role,isVisible,selector};
      });
    });
    webglContent=canvasClassification.filter(c=>c.role==='webgl-content');
    const overlays=canvasClassification.filter(c=>c.role==='overlay'||c.role==='background');
    const contentCanvases=canvasClassification.filter(c=>c.role==='content'&&c.isVisible);
    console.log(`     Canvases: ${canvasClassification.length} total (${webglContent.length} WebGL, ${overlays.length} overlay, ${contentCanvases.length} 2D, ${canvasClassification.filter(c=>c.role==='hidden').length} hidden)`);

    // Static PNG capture for 2D content canvases and overlays
    const canvases=await page.$$('canvas');
    for(const cc of[...contentCanvases,...overlays]){
      try{
        const du=await canvases[cc.i].evaluate(c=>{try{return c.toDataURL('image/png')}catch{return null}});
        if(du&&du.length>500)fs.writeFileSync(`${OUT}/images/canvas-${cc.i}.png`,Buffer.from(du.split(',')[1],'base64'));
        else await canvases[cc.i].screenshot({path:`${OUT}/images/canvas-${cc.i}.png`});
      }catch(e){}
    }

    // Pipeline-level WebGL capture: read framebuffer via toDataURL (preserveDrawingBuffer enabled)
    for(const wc of webglContent){
      ...
      for(let f=0;f<frameCount;f++){
        ...
        if(frame&&frame.length>2000){
          fs.writeFileSync(`${seqDir}/frame-${f+1}.png`,Buffer.from(frame.split(',')[1],'base64'));
          captured++;
        }
        await page.waitForTimeout(55);
      }
      console.log(`     WebGL[${wc.i}]: ${captured} frames captured (${wc.w}x${wc.h} ${wc.ctxType})`);
    }

    // Bundle analysis — scan ALL JS bundles, not just ones matching name patterns
    ...
    // Sort: largest bundles first (app bundles are bigger than vendor chunks)
    const jsSizes=[];
    for(const url of jsURLs.slice(0,15)){try{
    ...
    for(const{url,code}of jsSizes.slice(0,8)){
      fullBundleCode+=code+'\n';
      // GSAP detection: look for ScrollTrigger (reliable), gsap calls (minified names vary)
      ...
      // Also detect GSAP by its unique string signatures (works even when minified)
      ...
      // Frame sequence detection: framePath + frameCount + frameName pattern
      ...
      // Also detect sequential image patterns: /path/name-1.ext through /path/name-N.ext
      for(const m of code.matchAll(/["'](\/[^"']*?\/[^"']*?-)(\d+)(\.(?:webp|jpg|png))["']/g)){
        const basePath=m[1];const num=parseInt(m[2]);const ext=m[3];
        if(num<=2){
          // Check if there's a higher number nearby
          const highMatch=code.substring(m.index,m.index+500).match(new RegExp(basePath.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'(\\d+)'));
          if(highMatch&&parseInt(highMatch[1])>50){
            const existing=bundle.frameSequences.find(s=>s.path===basePath.substring(0,basePath.lastIndexOf('/')));
            if(!existing){
              bundle.frameSequences.push({path:basePath.substring(0,basePath.lastIndexOf('/')),name:basePath.substring(basePath.lastIndexOf('/')),count:parseInt(highMatch[1]),ext:ext.substring(1)});
            }
          }
        }
      }
    ...
    if(bundle.frameSequences.length)console.log(`     Frame sequences: ${bundle.frameSequences.length} (${bundle.frameSequences.map(s=>s.count+'f').join(', ')})`);
    ...
    if (bundle.frameSequences.length > 0) {
      ...
      for (const seq of bundle.frameSequences) {
        ...
        let dlCount = 0;
        // Download all frames in parallel batches of 10
    ...
    if (bundleLib.includes('gsap') || bundleLib.includes('scrolltrigger') || bundleLib.includes('lenis') || bundle.frameSequences.length > 0) {
      ...
      // ── Frame sequence canvases ──
      if (bundle.frameSequences.length > 0) {
        animScript += `// Frame sequence players (${bundle.frameSequences.length} sequences)\n`;
        ...
        animScript += `  const frames = [];\n`;
        animScript += `  let loaded = 0, currentFrame = 0;\n`;
        animScript += `  // Set canvas to match display size\n`;
        ...
        // ══ UNIVERSAL CANVAS BINDING — by classification, not index ══
        // Serialize classification into the client script so binding is dynamic
        animScript += `// Canvas classification: ${JSON.stringify(canvasClassification.map(c=>({i:c.i,role:c.role})))}\n`;
        animScript += `const _allCanvases=document.querySelectorAll("canvas");\n`;

        // Frame sequences: bind to canvases that are in matching sections
        // Strategy: find canvases whose parent section contains content matching the sequence
        if (bundle.frameSequences.length > 0) {
          animScript += `// Frame sequence binding (${bundle.frameSequences.length} sequences)\n`;
          // Give each canvas with a parent section a way to match
          animScript += `const _seqCanvases=[..._allCanvases].filter(c=>{\n`;
          animScript += `  const r=c.getBoundingClientRect();return r.width>0&&r.height>0;\n`;
          animScript += `}).filter(c=>!c.closest('[style*="pointer-events: none"],[style*="pointer-events:none"]'));\n`;
          // Filter to non-overlay visible canvases, then bind sequences in order
          bundle.frameSequences.forEach((seq, i) => {
            const useST = bundleLib.includes('gsap') || bundleLib.includes('scrolltrigger');
            animScript += `(()=>{\n`;
            animScript += `  const c=_seqCanvases[${i}];if(!c||c.getBoundingClientRect().width<10)return;\n`;
            animScript += `  const section=c.closest("section")||c.parentElement;if(!section)return;\n`;
            animScript += `  const obs=new IntersectionObserver(([e])=>{\n`;
            animScript += `    if(!e.isIntersecting)return;obs.disconnect();\n`;
            animScript += `    const player=createSequencePlayer(c,"${seq.path}","${seq.name}",${seq.count},"${seq.ext}");\n`;
            if (useST) {
              animScript += `    ScrollTrigger.create({trigger:section,start:"top bottom",end:"bottom top",scrub:1,\n`;
              animScript += `      onUpdate:s=>player.drawFrame(Math.round(s.progress*(${seq.count}-1)))});\n`;
            } else {
              animScript += `    window.addEventListener("scroll",()=>{const r=section.getBoundingClientRect();const p=Math.max(0,Math.min(1,(innerHeight-r.top)/(innerHeight+r.height)));player.drawFrame(Math.round(p*(${seq.count}-1)))},{passive:true});\n`;
            }
            animScript += `  },{threshold:0.1});\n`;
            animScript += `  obs.observe(section);\n`;
            animScript += `})();\n`;
          });
        }
      }

      // WebGL pipeline captures: bind to their original canvases (auto-play loop)
      if (webglContent && webglContent.length > 0) {
        animScript += `// WebGL pipeline captures (${webglContent.length} canvases)\n`;
        for (const wc of webglContent) {
          const seqDir = `${OUT}/sequences/webgl-${wc.i}`;
          if (!fs.existsSync(seqDir)) continue;
          const frameFiles = fs.readdirSync(seqDir).filter(f => f.endsWith('.png') || f.endsWith('.webp'));
          if (frameFiles.length < 5) continue;
          const fExt = frameFiles[0].split('.').pop() || 'png';
          // Find this canvas by its index in the DOM
          animScript += `(()=>{\n`;
          animScript += `  const c=_allCanvases[${wc.i}];if(!c||c.getBoundingClientRect().width<10)return;\n`;
          animScript += `  const section=c.closest("section")||c.parentElement;\n`;
          animScript += `  const player=createSequencePlayer(c,"/sequences/webgl-${wc.i}","/frame-",${frameFiles.length},"${fExt}");\n`;
          animScript += `  let f=0,last=0;function loop(t){if(t-last>83){player.drawFrame(f);f=(f+1)%${frameFiles.length};last=t}requestAnimationFrame(loop)}\n`;
          animScript += `  const obs=new IntersectionObserver(([e])=>{if(e.isIntersecting){obs.disconnect();requestAnimationFrame(loop)}},{threshold:0.05});\n`;
          animScript += `  obs.observe(section||c);\n`;
          animScript += `})();\n`;
        }
      }
    ...
    // Download videos — search ALL JS bundles for video paths
    const allVidPathsFromBundles = new Set();
    // Also search the fullBundleCode we already fetched
    ...
    const jsURLs2 = [...networkURLs].filter(u=>u.match(/\.js(\?|$)/i)).slice(0,8);
    for (const url of jsURLs2) {
      try {
        const code = await page.evaluate(async u=>{try{return await(await fetch(u)).text()}catch{return''}},url);
  ...
  // ══ UNIVERSAL CANVAS ASSEMBLY — replace by role, not index ══
  let ci=0;
  ...
  html=html.replace(/<canvas[^>]*>[\s\S]*?<\/canvas>/g,(match)=>{
    const idx=ci++;
    const cc=canvasClassification?canvasClassification[idx]:null;
    if(!cc){
      // No classification data — fallback to static image
    ...
    switch(cc.role){
      case 'overlay':
      case 'background':
        // Overlays: remove (they're effects like VHS filters that need JS)
        return '';
      case 'webgl-content':
        // WebGL content: keep canvas alive — animation script will drive it
        if(fs.existsSync(`${OUT}/sequences/webgl-${cc.i}`))return match;
        // No captured sequence — use static PNG
        if(fs.existsSync(`${OUT}/images/canvas-${cc.i}.png`))return`<img src="/images/canvas-${cc.i}.png" style="width:100%;height:auto"/>`;
        return match;
      case 'hidden':
        // Hidden canvases: keep as-is (might become visible via responsive CSS)
        return match;
      case 'content':
      default:
        // 2D content: replace with video if available, else static PNG
  ...
  // Empty JS-injected containers (Logo, etc) → inject video if available
  if(logoVid){
    html=html.replace(/<div class="[^"]*Logo_container[^"]*"><\/div>/g,
      `<div style="width:100%;aspect-ratio:750/110"><video autoplay muted playsinline loop style="width:100%;height:auto" src="${logoVid}"></video></div>`);
  }
...
async function main() {
  ...
  await context.addInitScript(() => {
    // PIPELINE INTERCEPT: force preserveDrawingBuffer on all WebGL contexts
