#!/usr/bin/env node
/**
 * Site X-Ray v18 — Universal precision cloner.
 * Builds on v17 with:
 *   - Create common stub files (favicon.ico, humans.txt, robots.txt) for broken link fixes
 *   - Map versioned/hashed CSS paths to downloaded local copies
 *   - Broader nav detection (any visible link cluster ≥3 links)
 *   - Better <picture>/<source> srcset rewriting for image fidelity
 *   - Generate stub files for broken CSS references in HTML
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v18-stable.js <url> [output-dir] [max-pages] [flags]
