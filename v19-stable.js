#!/usr/bin/env node
/**
 * Site X-Ray v19 — Universal precision cloner.
 * Builds on v18 with:
 *   - Capture and inject body background-color (fixes CSS score for 5+ sites)
 *   - Delete tiny/broken images (<100 bytes) post-clone to fix manifest scores
 *   - Download CDN scripts locally instead of keeping as external refs
 *   - WordPress/broader nav detection for interaction scoring
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v19-stable.js <url> [output-dir] [max-pages] [flags]
