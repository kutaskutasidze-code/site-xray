 ...
 *   - Canvas capture: viewport screenshot fallback for first full-viewport canvas on multi-canvas sites
 *   - Previous: content div visibility fix, textContent capture, font preloading, font-display:swap
 *
...
async function capturePage(page, urlPath, isFirst) {
  ...
  if (isFirst) { try {
    ...
    for(let i=0;i<canvases.length;i++){try{
      ...
      // v27: Use viewport screenshot for single-canvas pages (full-viewport WebGPU scene)
      // v40: Also allow for the FIRST canvas on multi-canvas pages IF it's truly full-viewport
      // (e.g. lusion.co has a full-viewport WebGL hero + small logo canvases)
      // Small canvases (logos, decorative) won't match the full-viewport threshold
      const isFullViewportCanvas = box && box.width >= 1200 && box.height >= 600;
      if(!captured&&viewportScreenshotPath&&fs.existsSync(viewportScreenshotPath)){
        if(canvases.length===1&&isLargeCanvas){
          fs.copyFileSync(viewportScreenshotPath,canvasPath);captured=true;
        } else if(i===0&&isFullViewportCanvas){
          fs.copyFileSync(viewportScreenshotPath,canvasPath);captured=true;
        }
      }
