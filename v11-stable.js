#!/usr/bin/env node
/**
 * Site X-Ray v11 — General start, precise finish.
 * Builds on v10 (multi-page + WebGL/3D) with:
 *   - Cookie/popup/overlay auto-dismissal
 *   - Full page scroll to trigger all lazy content
 *   - Wait for images + fonts to fully load
 *   - SVG sprite inlining (fixes broken logos)
 *   - Next.js image source resolution (highest quality)
 *   - Computed background-image capture (fixes hero sections)
 *   - Verification pass (screenshot comparison)
 *
 * Single file. One dependency (playwright). Zero config.
 *
 * Usage: node v11-stable.js <url> [output-dir] [max-pages]
...
if (!TARGET) {
  console.log(`Site X-Ray v11
Usage: node v11-stable.js <url> [output-dir] [max-pages] [flags]
...
Examples:
  node v11-stable.js https://example.com
  node v11-stable.js https://example.com ./output 50
  node v11-stable.js https://example.com --all
  node v11-stable.js https://example.com --auth auth-state.json
  node v11-stable.js https://example.com --save-auth
  node v11-stable.js https://example.com --interactive`);
  process.exit(0);
}
...
const MAX_PAGES = flags.all ? 999 : (parseInt(positional[2]) || 20);
...
async function capturePage(page, urlPath, isFirst) {
  ...
  // v11: Unified internal link rewriting — crawled pages → local, rest → original site
  const crawledSet = new Set([...crawled]);
  // Build lookup: path → local file
  const localPages = {};
  for(const c of crawledSet){ localPages[c]=pathToFile(c); localPages[c+'/']=pathToFile(c); }

  // (link rewriting moved to after all asset replacements — see below)

  // v11: Convert absolute same-domain hrefs to relative, then rewrite links
  // Must run AFTER all asset URL replacements
  // Convert all absolute same-domain references to relative
  const beforeCount = (html.match(new RegExp(DOMAIN.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'g'))||[]).length;
  html = html.split(DOMAIN + '/').join('/');
  html = html.split(DOMAIN + '"').join('/"');
  html = html.split(DOMAIN + "'").join("/'");
  const afterCount = (html.match(new RegExp(DOMAIN.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'g'))||[]).length;
  if(beforeCount > afterCount) console.log(`     v11: Converted ${beforeCount - afterCount} absolute refs to relative`);
  ...
  html=html.replace(/href="(\/[^"]*?)"/g,(match,linkPath)=>{
    ...
    if(localPages[linkPath])return`href="${localPages[linkPath]}"`;
    const clean=linkPath.replace(/\/$/,'');
    if(localPages[clean])return`href="${localPages[clean]}"`;
    if(localPages[clean+'/'])return`href="${localPages[clean+'/']}"`;
    return`href="${DOMAIN}${linkPath}" target="_blank"`;
  });
...
async function main() {
  ...
  if (flags.interactive || flags.saveAuth) {
    ...
    if (flags.saveAuth) {
      ...
      console.log(`   Reuse with: node v11-stable.js ${TARGET} --auth ${stateFile}\n`);
    }
  ...
  // ═══════════════════════════════════════
  // v11: Post-process — fix internal links across ALL pages
  // Now that all pages are crawled, we know the full set of local pages
  // ═══════════════════════════════════════
  ...
  for(const file of htmlFiles){
    ...
    h = h.split(DOMAIN + '"').join('/"');
    // Rewrite internal links: cloned → local file, non-cloned → original site
    h = h.replace(/href="(\/[^"]*?)"/g,(match,linkPath)=>{
      ...
      // Check all variants
      const clean = linkPath.replace(/\/$/,'');
      if(allLocalPages[linkPath])return`href="${allLocalPages[linkPath]}"`;
      if(allLocalPages[clean])return`href="${allLocalPages[clean]}"`;
      if(allLocalPages[clean+'/'])return`href="${allLocalPages[clean+'/']}"`;
      // Not cloned → original site in new tab
      return`href="${DOMAIN}${linkPath}" target="_blank"`;
    });
    if(h.length !== before){
  ...
  console.log(`     Fixed links in ${totalFixed}/${htmlFiles.length} pages (${allCrawled.size} local pages known)`);
